  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
      <div className="text-center mb-14">
        <span className="text-xs font-semibold text-primary-light uppercase tracking-widest">联系方式</span>
        <h1 className="text-3xl sm:text-4xl font-bold text-text-primary mt-3 mb-3">联系我</h1>
        <p className="text-text-secondary text-sm max-w-md mx-auto">
          如果您有 WebGL 游戏开发需求或希望合作，欢迎通过以下方式联系
        </p>
      </div>

      {/* Contact cards */}
      <div className="grid sm:grid-cols-3 gap-5 max-w-3xl mx-auto mb-20">
        {[
          {
            icon: (
          ...
          <div
            key={item.title}
            className="group bg-bg-card rounded-2xl p-6 border border-border/60 glow-on-hover hover:-translate-y-1 text-center"
          >
            <div className="w-14 h-14 bg-primary/10 rounded-2xl flex items-center justify-center mx-auto mb-5 text-primary-light group-hover:bg-primary/20 group-hover:scale-110 transition-all duration-300">
              {item.icon}
            </div>
            ...
            {item.href ? (
              ...
              <p className="text-primary-light text-sm font-medium">{item.action}</p>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-10">
          <h3 className="text-2xl font-bold text-text-primary">合作流程</h3>
        </div>
